...
def bleu_scorer(source: np.ndarray, predicted: np.ndarray, actual: np.ndarray, target_tokenizer, source_tokenizer):
    ...
    actual_sentences = []
    source_sentences = []
    for s, (a, b) in zip(source, zip(predicted, actual)):
        # words_predicted = target_tokenizer.decode(a, skip_special_tokens=True).split() # T5
        # words_actual = target_tokenizer.decode(b, skip_special_tokens=True).split() # T5
        # words_source = source_tokenizer.decode(s, skip_special_tokens=True).split() # T5

        words_predicted = target_tokenizer.decode(a).split()
        words_actual = target_tokenizer.decode(b).split()
        words_source = source_tokenizer.decode(s).split()
